...
// SSID & Password
const char* ssid = "Yazan";  // Enter your SSID here
...
void setup() {
  ...
  if(!Stop){
    ...
    request->send(SPIFFS,"/stop.html");
  });
    }
  else {server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS,"/user_interface.html");
  });
  }
        ...
        if(LoadWavFileHeader("/flute_16_16.wav", &Wav2)){
            if( LoadWavFileHeader("/guitar_16_16.wav", &Wav3)){
                if( LoadWavFileHeader("/piano_16_16.wav", &Wav4)){
                    return LoadWavFileHeader("/bongo_16_16.wav", &Wav5);
                }
            }
        }
    }         
